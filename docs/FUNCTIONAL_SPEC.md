...
bug]** and collected in §12; the rewrite should fix these rather than
...
split** (§5.6) and the **price-service isolation of external API quotas**.
...
| **Morningstar HTML test fixtures** (`morningstar_test.go` contains captured Morningstar page content) | Morningstar's copyrighted material. | Do not carry over. The scraper itself is dead (§6.3) and is not part of this spec's required scope. |
| Morningstar scraping (runtime) | ToS concern (automated collection) and the scraped endpoints are long dead. | Dropped from the rewrite; see §6.3 for the replacement requirement. |
...
**Branding note:** the legacy app's identity is "Ω: Portfolio Manager" with
an omega logo. The name can stay; the omega *artwork* cannot. A simple
original Ω glyph rendered from a font, or a fresh design, is fine.
...
- Shares still held per lot = purchased quantity − Σ shares sold from that
...
- Lot cost basis = stillHeld × purchasePricePerShare + purchaseCosts
...
- Current value of a position = shares held × latest price (short/long split
  maintained). Gain = current value − basis.
- Account investment value = Σ its positions' current values. Broker totals
  = Σ account sweeps and Σ account investment values. Grand totals across
...
- Per-sale gain = (salePricePerShare − purchasePricePerShare) × sharesSold
...
- **Current allocation**: each priced position's total value is distributed
  ...
  "Sweeps" position. Portfolio total = Σ class buckets.
...
- **Drift**: per class, current fraction = classValue / portfolioTotal;
  target quantity = portfolioTotal × targetFraction; delta = target −
...
- The user's tentative trades convert to per-class dollar allocations.
  **Mutual funds are bought in dollars** (shares derived = value/price);
  ...
  shares×price). This distinction is load-bearing in the math and the buy
  dialog.
- Per class the planner reports: current, post-trade ("rebalance"), and
  ...
  (⌊need / (price × classFraction)⌋) and cost, ordered by concentration.
...
- Sell-side planning was a stub in legacy; it is **optional** in the
  rewrite (the UI slot exists — see §9.14).
...
- Time-value conversion: `amount × Index(destination)/Index(origin)`.
...
FRED CPIAUCSL CSV (see §5.7). Embed a snapshot for offline startup; refresh
...
1. **Manual entry (required, the baseline)**: adding a security takes just
   ...
   The edit-weights forms in §9.10 cover asset class, sector, and market
...
`/updatemarketcapweights` — extend per §6.3); manual price-history CRUD for
...
FAB: add broker. Needs a way to reveal hidden brokerages (new — see §4.3).
...
(links to lot detail, §9.7), **Sparkline** (6-month adjusted-close trend),
**Shares**, **Basis**, **Current Value**, **ST Gain**, **LT Gain**. Pie
"Total Holdings by Security" (clickable). Actions: FAB add → Buy dialog
(§9.8) with the account preselected; FAB edit account; FAB delete account
(enabled only when empty); "Hide this empty account" button when empty.

### 9.6 All positions (nav: Positions)

Same table and pie as §9.5 but portfolio-wide; tickers link to the
security-holdings page (§9.11). Default sort: current value descending.
...
**Sell** — enabled when ≥1 lot is checked — opens the Sell dialog (§9.9).
...
as §9.7 plus an **Account** column. Actions: **Security Details** → §9.10;
...
Without Investing"). Table: **Asset Class** (links to §9.15), **Total
...
Card "Positions in {assetClass}". Table: **Ticker** (links to §9.10),
...
Card "Securities". Table: **Ticker** (links to §9.10), **Sparkline**,
**Description**. FAB: add security (§9.18). Needs a way to reveal hidden
securities (new).

### 9.18 Add security

Card "Add New Security". **Ticker** (required), Cancel/Submit. Post-add,
classification data is entered manually via §9.10's edit forms (see §6.3 —
...
1. Logout didn't invalidate the operative auth cookie; cookie flags weak
   (§3.1). Google signing keys cached forever (§3.1).
2. No user allowlist — any Google account got a portfolio (§3.2).
3. Money/dates stored as formatted strings; no DB-side ordering (§4.3).
4. Global (not scoped) uniqueness of tickers and account nicknames (§4.3).
5. Tax-report query not portfolio-scoped (§5.3). Two different long-term
   thresholds (§5.1).
6. Unhide endpoints unimplemented; hidden items unreachable (§4.3).
...
11. Sparkline N+1 fetches (§8.3). Sign-in state lost on page reload (§3.1).
12. Price-server concurrency bugs: worker-goroutine leak on coalesced
    ...
    pending-request map (§6.1). IEX adjusted-close JSON tag typo made all
    adjusted closes zero.
13. CPI fetched once at startup, process dies offline (§10); inflation
    applied in opposite directions by two code paths (§5.7).
14. Snackbar typo "position deteted"; seed-data country typos (§4.2).
15. In-process caches without locks or invalidation (§10).
