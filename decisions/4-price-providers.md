...
FUNCTIONAL_SPEC §6.1, docs/design/initial-build-scope.md §7.
...
| **EODHD** | Yes — EOD API includes adjusted close (splits+dividends), splits and dividends endpoints | **Yes** (MF NAVs next morning) | 20 calls/day, **past year only** | €19.99/mo "All World" removes both limits | **Fallback** |
...
| Yahoo/scraping | — | — | — | — | Excluded on the same ToS grounds as Morningstar (spec §6.3) |
...
- **Tiingo** is a near-perfect fit: its EOD response is the spec's
  ...
  one request, and 250 calls/day is ~10× this app's ceiling. Free.
- **EODHD** as the second provider: same field coverage including
  ...
  first-fetch of a new ticker), €19.99/mo removes the limits; nothing
